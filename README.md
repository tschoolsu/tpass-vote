...
envelope 加密，沒開票私鑰誰都打不開）、**外信封＝名冊上的一枚旗標**（`Voter.hasVoted`，
只記有無、不記時間；票匭那一列從頭到尾沒有任何欄位指得回你，一人一票、送出後不可更改）、
**開票＝先拆外信封**
（彌封：把票匭密碼學洗牌成公開的快照＋雜湊承諾，原表整張刪除），選委再於**瀏覽器本地**載入
...
| 攻擊者 | 能否知道誰投給誰 |
| --- | --- |
| 應用程式碼、`pg_dump` 邏輯備份外洩 | ❌ 連對應關係都不存在於任何可查詢的欄位：票匭只有 `id`／`electionId`／`ciphertext`，名冊只有 `hasVoted` 布林。`pg_dump` 不輸出系統欄位，所以拿到備份的人手上沒有任何可以 join 的東西 |
| 持開票金鑰的選委（無 DB 權限） | ❌ 有鑰匙但拿不到「誰對應哪個密文」（票匭沒有身分欄位，彌封後又已洗牌） |
| 脅迫者 | ❌ **沒有救濟**。一人一票、送出後不能改，受迫投完就定案了。舊版可換裝置重投來化解脅迫，但那要求 DB 全程留著 `voterId↔密文`；本系統選擇犧牲反脅迫來換取「連結從一開始就不存在」。另外 §26-1 Ⅳ 要求公開代碼↔選票內容，收據被搶走就證明得了內容——這部分是法規要求的可驗證性帶來的，不是實作疏漏 |
| 已關票之後才拿到 DB／備份的人 | ❌ Postgres 每列都有隱藏系統欄位 `xmin`（寫入該列的交易 id），而 `Voter.hasVoted` 與票是同一個交易寫的，所以 `JOIN ON b.xmin = v.xmin` 本來對得起來。**關票（`advanceStatus` 推進到 `closed`）時會把整場的名冊與票匭各原值改寫一次**，全部的列從此帶同一個 xid，那句 join 吐出 N×N 的組合、資訊量歸零 |
| 投票**進行中**就在讀資料庫的人 | ⚠️ **仍可還原一小部分。** 收票時會順便原值改寫 K 列不相干的資料（`PERTURB_K`，預設 16），讓同一個 xid 落在 K+1 個人與 K+1 張票上；但那是稀釋不是消滅——後來的票會覆蓋前面留下的 xid，侵蝕到最後**約 1.5% 的選舉人**會剩下「這個 xid 只對到你一個人、也只對到一張票」，那一票就被精確還原（比例與名冊大小無關，3000 人的場次約 44 人）。關票後這些殘留會被上面那次塌縮清掉，所以危險窗口只有「投票進行中」。實體備份／WAL 逐筆解碼同樣擋不住。真正的對策是存取控制：**投票期間不得對正式庫下互動式查詢**（見 SOP），並維持金鑰分持讓單一持鑰者拿不到完整私鑰 |
| 彌封前有 `EncryptedBallot` 讀權限者 | ❌ **已封堵**（2026-09-08 拿掉 `voterId`）。可回溯代碼由投票人瀏覽器產生、封在加密選票內部，伺服器與 DB 都只看得到密文，沒有開票私鑰就算不出代碼；現在連 `voterId↔密文` 本身也不存在，無從建對照表去跟公開的 disclosures CSV join。代價是伺服器失去「代碼集合與票匭相符」這項對帳，只剩張數相符；結果造假的防線改為兩位選委各自獨立開票比對（見 SOP）。投票期間仍建議把 `t_vote` 排除在每日備份外，但那已不是承重的對策，只是衛生習慣 |
...
> §26-1 Ⅷ 一「重複投票者選票無效」：本系統一人一票，第二次送出會在鎖內被直接拒絕
>（`Voter.hasVoted`），票匭裡不可能出現同一人的兩張票，故不構成重複投票。
