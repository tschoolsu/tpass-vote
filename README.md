...
| **單獨一位有 `EncryptedBallot` 讀權限者（不需要金鑰、不需要合謀）** | ⚠️ 僅限**彌封前**。`receiptOf()` 只是密文的 SHA-256 前 12 碼，不需要開票私鑰就能自己對每一筆 `(voterId, ciphertext)` 算出代碼；結果公告後 `/api/elections/<slug>/disclosures` 是免登入即可下載的公開 CSV（代碼↔去識別化票意），兩者用代碼 join 就能完整還原「誰投給誰」。這裡擋不住的是「誰能建對照表」，職權分離（金鑰在選委會、DB 在維運）分離的是「誰能解密」，**對這個攻擊面無效**。真正的緩解：投票期間排除 `t_vote` 的每日備份，並在彌封時如實刪除 `EncryptedBallot`（已實作）——一旦彌封＋備份也清掉，連結就不存在，任何人事後回推不了 |
