...
export async function advanceStatus(electionId: string, reason?: string): Promise<ActionResult> {
  ...
  if (next === "voting") {
    ...
    // D2-4／D12-2：上面只驗過排程「跨度」≥48 小時，沒驗「按下這一刻到截止還剩多少」——
    ...
    // 剩餘時數一起記進 audit log。
    // 顯示用的剩餘時數無條件捨去（不用 Math.round）：這裡一定是「不足 48 小時」分支，
    // 47.98 捨去成 47.9 才會讓訊息裡的數字也小於 48——四捨五入會把它印成 48.0，跟後面
    // 「法定投票期間為 48 小時」擺在一起變成自相矛盾、選委看不出到底哪裡不足。
    const remainingMs = votingEndsAt.getTime() - Date.now();
    if (remainingMs < MIN_VOTING_HOURS * 3_600_000) {
      const trimmedReason = reason?.trim();
      const remainingHours = Math.floor((Math.max(remainingMs, 0) / 3_600_000) * 10) / 10;
      ...
      votingForceRemainingHours = remainingHours;
    }
